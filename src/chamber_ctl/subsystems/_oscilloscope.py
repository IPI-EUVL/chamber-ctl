...
class ScopeReader:
    ...
    def capture_burst_and_read(self, N=200):
        ...
        self.__read_queue.put((self.last_start_cmd, desc, data)) # signal read thread to proceed

    def __proc_thread(self, stop_flag: StopFlag):
        while stop_flag.run():
            start, desc, data = self.__read_queue.get() # wait for signal to read
            ...
            self.__out_queue.put((start, data, indexes)) # signal for output
    ...
    def __proc_thread(self, stop_flag: StopFlag):
        ...
        while stop_flag.run():
            ...
            if not self.__in_queue.empty():
                start, data, indexes = self.__in_queue.get() # wait for signal of new data
            ...
            time.sleep(0.1)
"""
class OscilloscopeSubsystem:
    def __init__(self, address):
        self.__osc_queue = queue.Queue()
        self.__out_queue = queue.Queue()

        self.osc = ScopeReader(address)
