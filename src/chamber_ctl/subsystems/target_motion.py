...
class MotionSegment:
    def __init__(self, t_l, t_r, v_l, v_r=None):
        self.lin_target = t_l
    ...
    @staticmethod
    def by_time(t_l, t_r, p_l, p_r, time):
        v_l = abs((t_l - p_l) / time) if time != 0 else 0.0
        v_r = abs((t_r - p_r) / time) if time != 0 else 0.0
...
class TargetMotionProfile:
    ...
    def get_length(self):
        ...
        for segment in self.__segments:
            len_l = abs(
                (segment.lin_target - c_l) / segment.lin_velocity
                if segment.lin_velocity != 0
                else 0.0
            )
            len_r = abs(
                (segment.rot_target - c_r) / segment.rot_velocity
                if segment.rot_velocity != 0
                else 0.0
            )
    ...
    def move_len(self, segment_num: int):
        (start_l, start_r) = (
            (
                self.__segments[segment_num - 1].lin_target,
                self.__segments[segment_num - 1].rot_target,
            )
            if segment_num > 0
            else (0.0, 0.0)
        )
        ...
        end_l, end_r = (
            self.__segments[segment_num].lin_target,
            self.__segments[segment_num].rot_target,
        )

        len_l = abs(
            (end_l - start_l) / self.__segments[segment_num].lin_velocity
            if self.__segments[segment_num].lin_velocity != 0
            else 0.0
        )
        len_r = abs(
            (end_r - start_r) / self.__segments[segment_num].rot_velocity
            if self.__segments[segment_num].rot_velocity != 0
            else 0.0
        )
    ...
    def segment_at(self, time: float):
        ...
        for i, segment in enumerate(self.__segments):
            len_l = abs(
                (segment.lin_target - cur_l) / segment.lin_velocity
                if segment.lin_velocity != 0
                else 0.0
            )
            len_r = abs(
                (segment.rot_target - cur_r) / segment.rot_velocity
                if segment.rot_velocity != 0
                else 0.0
            )
...
class MotionState:
    ...
    def get_current_motion_command(self):
        ...
        return (
            seg.lin_target + self.__current_rep * self.__profile.get_end_position()[0],
            seg.rot_target + self.__current_rep * self.__profile.get_end_position()[1],
            seg.lin_velocity,
            seg.rot_velocity,
        )
    ...
    def get_current_position(self):
        return self.get_position_at_time(
            self.__current_position + self.__profile.get_length() * self.__current_rep
        )
    ...
    def get_position_at_time(self, time: float):
        ...
        l_pos, r_pos = self.__profile.get_position_at_time(
            time % self.__profile.get_length()
        )

        # print("Current repetition:", reps)

        l, r = (
            l_pos + reps * self.__profile.get_end_position()[0],
            r_pos + reps * self.__profile.get_end_position()[1],
        )
    ...
    def update_position(self, l_pos: float, r_pos: float):
        ...
        off_l, off_r = (
            l_pos - self.__current_rep * self.__profile.get_end_position()[0],
            r_pos - self.__current_rep * self.__profile.get_end_position()[1],
        )
        segment_time = self.__profile.time_at_posion(
            off_l, off_r, self.__current_segment
        )
    ...
    def get_time_until_end_of_segment(self):
        ...
        seg_end_time = self.__profile.segment_begin_time(
            self.__current_segment
        ) + self.__profile.move_len(self.__current_segment)
    ...
    def get_current_time(self):
        return (
            self.__current_position + self.__current_rep * self.__profile.get_length()
        )
    ...
    def encode(self):
        b_my_data = segment_bytes.encode(
            [
...
class TargetMotionConfig:
    def __init__(self, max_l_size: float):
        ...
        self.limit_speed_l = 3.0
        self.limit_speed_r = 2.0

    def encode(self):
        return segment_bytes.encode(
            [
                struct.pack("d", self.max_l_size),
                struct.pack("d", self.traverse_speed_l),
                struct.pack("d", self.traverse_speed_r),
                struct.pack("d", self.limit_speed_l),
                struct.pack("d", self.limit_speed_r),
            ]
        )
    ...
    def decode(b_data: bytes):
        ...
        config = TargetMotionConfig(struct.unpack("d", segments[0])[0])
        config.traverse_speed_l = struct.unpack("d", segments[1])[0]
        config.traverse_speed_r = struct.unpack("d", segments[2])[0]
        config.limit_speed_l = struct.unpack("d", segments[3])[0]
        config.limit_speed_r = struct.unpack("d", segments[4])[0]
...
class TargetMotion:
    ...
    def move_to_position(
        self, l_pos: float, r_pos: float, speed_l: float, speed_r: float
    ):
        pass
